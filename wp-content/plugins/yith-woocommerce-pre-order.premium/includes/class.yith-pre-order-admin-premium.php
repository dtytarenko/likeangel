...
if (!class_exists('YITH_Pre_Order_Admin_Premium')) {
	...
	class YITH_Pre_Order_Admin_Premium extends YITH_Pre_Order_Admin {
		...
		public function action_links( $links ) {
			$links = yith_add_action_links( $links, $this->_panel_page, true, YITH_WCPO_SLUG );
			return $links;
		}
		...
		public function add_pre_order_column_content_orders( $column_name, $post_id ) {
			if ( $column_name == 'pre_order' ) {
				...
				if ( 'yes' != $order_has_preorder || 'cancelled' == $order->get_status() ) {
					return;
				}
				...
				if ( $pre_order_dates ) {
					...
					foreach ( $pre_order_dates as $date ) {
						...
						if ( empty( $date['date'] ) ) {
							...
							$array_data['date'] = get_date_from_gmt( date( 'Y-m-d H:i:s', $date['date'] ), apply_filters( 'yith_ywpo_date_format', 'Y/m/d H:i', $post_id ) );
						}
		...
		public function premium_settings_options( $settings ) {
			...
			$settings['option3'] = array(
				...
				'type'    => 'checkbox',
				'desc'    => esc_html__( 'By enabling this option, Pre-Order products with no stock can be purchased.',
                    'yith-pre-order-for-woocommerce' ),
