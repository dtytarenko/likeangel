...
if ( ! class_exists( 'YITH_Pre_Order_Edit_Product_Page_Premium' ) ) {
	...
	class YITH_Pre_Order_Edit_Product_Page_Premium extends YITH_Pre_Order_Edit_Product_Page {
		...
		public function update_settings( $post_id ) {
			...
			if ( 'yes' == $is_pre_order ) {
				...
				if ( ! empty( $previous_sale_date ) && ! empty( $new_sale_date ) ) {
					if ( $new_sale_date !== $previous_sale_date ) {
						...
						WC()->mailer();
						$customers = YITH_Pre_Order_Scheduling::get_pre_order_customers( $post_id, $new_sale_date );
						if ( $customers ) {
			...
			$pre_order_no_date_label = isset( $_POST['_ywpo_preorder_no_date_label'] ) ? wc_clean( $_POST['_ywpo_preorder_no_date_label'] ) : '';
			$pre_order->set_pre_order_no_date_label( $pre_order_no_date_label );
		...
		public function pre_order_tab_content() {
			...
			<div id="preorder_product_data" class="panel woocommerce_options_panel hidden">
				<div class="options_group hide_if_external">
					...
					woocommerce_wp_text_input( array(
						'id'          => '_ywpo_preorder_no_date_label',
						'label'       => esc_html__( 'No date label', 'yith-pre-order-for-woocommerce' ),
						'desc_tip'    => 'true',
						'description' => esc_html__( 'Message to be shown when no date is set. Leave it empty to use the global option.',	'yith-pre-order-for-woocommerce' ),
						'placeholder' => esc_html__( 'Coming soon...', 'yith-pre-order-for-woocommerce' )
					) );
				...
				if ( ( $action == 'ywpo-set-pre-order' ) || ( $action == 'ywpo-remove-pre-order' ) ) {
					...
					foreach ( $post_ids as $post_id ) {
						...
						if ( $pre_order->product->is_type( 'simple' ) ) {
							$pre_order->set_pre_order_status( $action == 'ywpo-set-pre-order' ? 'yes' : 'no' );
							...
							foreach ( $children as $child ) {
								...
								$pre_order_variation->set_pre_order_status( $action == 'ywpo-set-pre-order' ? 'yes' : 'no' );
							}
						}
					}
		...
		public function woocommerce_product_after_variable_attributes( $loop, $variation_data, $variation ) {
			...
			<div class="show_if_variation_pre_order">
				<div>
					...
					$pre_order_no_date_label = $pre_order->get_pre_order_no_date_label();
					?>
                    ...
                    <p class="form-row form-row-first">
                        ...
                        <input type="text" size="30" name="<?php echo esc_attr( '_ywpo_preorder_availability_date_label[' . $loop . ']' ); ?>"
                               placeholder="<?php echo esc_html__( 'Available on: {availability_date} at {availability_time}', 'yith-pre-order-for-woocommerce' ); ?>"
                               value="<?php echo esc_attr( $pre_order_availability_date_label ); ?>"
                        />
                    </p>

                    <p class="form-row form-row-last">
                        <label>
							<?php echo esc_html__( 'No date label', 'yith-pre-order-for-woocommerce' );
							echo wc_help_tip( esc_html__( 'Message to be shown when no date is set. Leave it empty to use the global option.', 'yith-pre-order-for-woocommerce' ) ); ?>
                        </label>
                        <input type="text" size="30" name="<?php echo esc_attr( '_ywpo_preorder_no_date_label[' . $loop . ']' ); ?>"
                               placeholder="<?php echo esc_html__( 'Coming soon...', 'yith-pre-order-for-woocommerce' ); ?>"
                               value="<?php echo esc_attr( $pre_order_no_date_label ); ?>"
                        />
                    </p>
				...
				<fieldset class="form-row form-row-first">
					<p>
						<?php echo esc_html__( 'Price adjustment', 'yith-pre-order-for-woocommerce' );
				...
				<fieldset class="form-row form-row-first hide_if_manual">
					<p>
						<?php echo esc_html__( 'Adjustment type', 'yith-pre-order-for-woocommerce' );
		...
		public function save_variable_fields( $post_id, $_i ) {
			...
			$pre_order_no_date_label = isset( $_POST['_ywpo_preorder_no_date_label'][ $_i ] ) ? wc_clean( $_POST['_ywpo_preorder_no_date_label'][ $_i ] ) : '';
			$pre_order->set_pre_order_no_date_label( $pre_order_no_date_label );
