         ...
         case "valid":
            ...
            <div > <?php /*class="wpwoof-aligncenter"*/ ?>
                ...
                if($case_status=="expired"){
                    ?><br><p style="font-weight: bold;">Your license is not active: License has expired. <a target="_blank" href="<?php echo WPWOOF_SL_STORE_URL; ?>/checkout/?edd_license_key=<?php echo trim(get_option( 'pcbpys_license_key' ));?>&utm_campaign=admin&utm_source=licenses&utm_medium=renew">Please renew your license.</a> If you still see this error message after renewal please use "Update the license status" button.</p><?php
                } elseif($case_status=="disabled"){
                    ?><br><p style="font-weight: bold;">Your license is not active<?=isset($license_data->license_status)?': <span style="color: red">'.$license_data->license_status.'</span>':''?>.</p><?php
                }
                ?>
                <input type="submit" class="wpwoof-button wpwoof-button-blue" name="pcbpys_license_deactivate" value="<?php _e('Deactivate License'); ?>"/>
                <input type="submit" class="wpwoof-button wpwoof-button-blue" name="pcbpys_license_update" value="Update the License status"/>
            </div>
            </form>
            <?php
