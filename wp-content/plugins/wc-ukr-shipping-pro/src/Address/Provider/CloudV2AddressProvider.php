...
class CloudV2AddressProvider implements AddressProviderInterface
{
    private const API_URL = 'https://api.wcukraineshipping.com/v2/addresses/';
    ...
    {
        $response = $this->sendRequest('searchCities', [
            'mode' => 'byQuery',
            'query' => $query,
        ]);
    ...
    public function searchWarehousesByQuery(
        ...
        $response = $this->sendRequest('searchWarehouses', $payload);
    ...
    public function searchWarehouseByRef(string $ref): ?Warehouse
    {
        ...
        $response = $this->sendRequest('searchWarehouses', [
            'mode' => 'byRef',
            'ref' => $ref,
        ]);
    ...
    private function sendRequest(string $uri, array $payload)
    {
        return wp_remote_post(self::API_URL . $uri, [
            'headers' => [
                'Content-Type' => 'application/json',
                'Wcus-License-Key' => $this->getLicenseKey(),
    ...
    private function getLicenseKey(): string
    {
        ...
        if (empty($key)) {
            throw new LicenseNotFoundException("Unable to find license key");
        }
