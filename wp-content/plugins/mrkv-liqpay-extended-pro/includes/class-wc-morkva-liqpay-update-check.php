...
class PluginUpdateChecker_2_1 {
	...
	protected function installHooks(){
		...
		add_action('after_plugin_row', array($this, 'mrkv_add_custom_message'), 10, 3);

		$mrkv_licence_management_api = get_option('mrkv_licence_management_api');

		if(!$mrkv_licence_management_api){
			return array();
		}
	 ...
	 * Add custom message
	 * */
	public function mrkv_add_custom_message($plugin_file, $plugin_data, $status)
	{
		if(str_contains($plugin_file, $this->pluginFile))
	    {
	        $mrkv_license_management_api = get_option('mrkv_licence_management_api');

	        if(!$mrkv_license_management_api)
	        {
	        	echo '<style>tr[data-plugin="' . $this->pluginFile .'"] td, tr[data-plugin="' . $this->pluginFile .'"] th{box-shadow: unset !important;}</style>';
	        	echo '<tr class="plugin-update-tr active" id="' . $this->slug .'-update" data-slug="' . $this->slug .'" data-plugin="' . $this->slug .'/' . $this->slug .'.php"><td colspan="4" class="plugin-update colspanchange"><div class="update-message notice inline notice-warning notice-alt"><p>Automatic update and support is unavailable for this plugin. <a target="blanc" href="/wp-admin/admin.php?page=morkva-licence-management">Click here to activate your license</a>.</p></div></td></tr>';
	        }
	        else{
	        	$result = file_get_contents("https://morkva.co.ua/wp-json/licenseManagement/v2?license_management=" . get_option('mrkv_licence_management_api'));

	        	$plugin_file_main = strtok($plugin_file, '/');
	        	...
	        	if($result)
	        	{
		            ...
		            && is_array($plugins['plugins']) && isset($plugins['plugins'][$plugin_file_main]) 
		            && isset($plugins['plugins'][$plugin_file_main]['enabled']) && $plugins['plugins'][$plugin_file_main]['enabled'] == 'on')
		          {
		          }
		          else{		          	
		          	echo '<style>.plugin-data-mrkv-' . $this->slug .' + tr.plugin-update-tr{ display:none !important; }tr[data-plugin="' . $this->pluginFile .'"] td, tr[data-plugin="' . $this->pluginFile .'"] th{box-shadow: unset !important;}</style>';
	        		echo '<tr class="plugin-update-tr active plugin-data-mrkv-' . $this->slug .'" id="' . $this->slug .'-update" data-slug="' . $this->slug .'" data-plugin="' . $this->slug .'/' . $this->slug .'.php"><td colspan="4" class="plugin-update colspanchange"><div class="update-message notice inline notice-error notice-alt"><p>Automatic update and support is unavailable for this plugin. <a target="blanc" href="/wp-admin/admin.php?page=morkva-licence-management">Click here to activate your license</a>.</p></div></td></tr>';
		          }
	        	}
	        }
	    }
