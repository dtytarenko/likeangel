...
class Product_Attribute_Updater {
	...
	private static function handleAttributeChange($product, $attr_key_name, $products_changes, $data, $product_id, $attributes): array {
		...
		// DEVSTUFF: If visible_fp is not set, this means the option was not changed. In that case need to get and use
		// DEVSTUFF: the old value in the new attribute object that will be created.
		if (!isset($visible_fp)) {
			// DEVSTUFF: Remove the prefix 'attribute_' from the attribute name, e.g. from 'attribute_pa_color'
			// DEVSTUFF: because in the product object the name of the attribute is without the prefix 'attribute_'
			$attr_key_name_inside_product_object = preg_replace('/^attribute_/', '', $attr_key_name);
			if (isset($product->get_attributes()[$attr_key_name_inside_product_object])) {
