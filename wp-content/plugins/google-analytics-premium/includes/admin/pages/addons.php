...
function monsterinsights_get_addons() {
    ...
    // Check if $addon->categories is an array
    $addon_categories = isset($addon->categories) && is_array($addon->categories) ? $addon->categories : array();

    // Determine whether the user is licensed to use this Addon or not.
    if (
        empty( $type ) ||
        ( in_array( 'Pro', $addon_categories ) && ( $type != 'pro' && $type != 'master' ) ) ||
        ( in_array( 'Plus', $addon_categories ) && $type != 'plus' && $type != 'pro' && $type != 'master' ) ||
        ( in_array( 'Basic', $addon_categories ) && ( $type != 'basic' && $type != 'plus' && $type != 'pro' && $type != 'master' ) )
    ) {
        // Unlicensed
